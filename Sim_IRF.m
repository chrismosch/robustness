% Repeatedly simulates the impulse responses to a shock while theta changes each period.
...
mtype = {'Approximating model','Worst-case model'};
  
% IRFs
figure(1);
  for i=1:nvar                             % for each variable
    subplot(nvar,2,(i-1)*2+1)              % approximating model
      plot(0:Tsim-1, Mean_a(:,i), '-k');
      hold on
      plot(0:Tsim-1, Mean_a(:,i)+2*Std_a(:,i), ':k');
      plot(0:Tsim-1, Mean_a(:,i)-2*Std_a(:,i), ':k');
      hold off
      ylabel(vnames(i))
      if i==1
        title(mtype(1))
        legend('mean','\pm2std',4);
      end
    subplot(nvar,2,(i-1)*2+2)              % worst-case model
      plot(0:Tsim-1, Mean_w(:,i), '-k');
      hold on
      plot(0:Tsim-1, Mean_w(:,i)+2*Std_a(:,i), ':k');
      plot(0:Tsim-1, Mean_w(:,i)-2*Std_a(:,i), ':k');
      hold off
      if i==1
        title(mtype(2))
...
% histograms for IRF for period 1 after shock

% with summary statistics
figure(2);
  for i=1:nvar                                             % for each variable: y (i=1), pi (i=2), interest rate (i=3) 
    % approximating model
    subplot(nvar,4,(i-1)*4+1)                              % histogram
      temp_a = reshape(SimR_a(2,i,:),[length(SimR_a),1]);  % reshapes IRF for variable i into (Trep x 1) vector
      hist(temp_a)
      ax = gca; 
      set(ax,'Ygrid','on');
    ...
    subplot(nvar,4,(i-1)*4+2)                              % summary statistics
        ...
        axis off
    % worst-case model
    subplot(nvar,4,(i-1)*4+3)                              % histogram          
      temp_w = reshape(SimR_w(2,i,:),[length(SimR_w),1]);  % reshapes IRF for variable i into (Trep x 1) vector
      hist(temp_w)
      ax = gca; 
      set(ax,'Ygrid','on');
      h = findobj(gca,'Type','patch');
    ...
    subplot(nvar,4,(i-1)*4+4)                              % summary statistics
...
% with summary statistics but only interest rate
%{
i = 3;
figure(3);
% approximating model
  subplot(2,2,1)                                           % histogram
    temp_a = reshape(SimR_a(2,i,:),[length(SimR_a),1]);    % reshapes IRF for variable i into (Trep x 1) vector
    hist(temp_a)
    ax = gca; 
    set(ax,'Ygrid','on');
    ...
    set(h,'FaceColor',[192/255 192/255 192/255]);
    title(mtype(1))
    subplot(2,2,2)                                         % summary statistics
        descr = {['Mean:' '          ' num2str(Mean_a(2,i))];...
                 ['Median:' '       ' num2str(Median_a(2,i))];...
                 ['Min:' '            ' num2str(Min_a(2,i))];...
                 ['Max:' '           ' num2str(Max_a(2,i))];...
    ...
    subplot(2,2,3)                                         % histogram          
      ...
      set(h,'FaceColor',[192/255 192/255 192/255]);
      title(mtype(2))
    subplot(2,2,4)                                         % summary statistics
        descr = {['Mean:' '          ' num2str(Mean_w(2,i))];...
                 ['Median:' '       ' num2str(Median_w(2,i))];...
                 ['Min:' '            ' num2str(Min_w(2,i))];...
                 ['Max:' '           ' num2str(Max_w(2,i))];...
